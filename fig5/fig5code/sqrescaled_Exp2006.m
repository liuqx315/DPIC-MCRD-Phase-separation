...
pick_num=round(linspace(40,240,num));
y = ones(num, 1);
pick_num=pick_num'.*y;
...
% save('Exp2006.mat','T','Sq_rt','Kmax','SqQmax2','Q_Qmax') ;
%%
data=load("Exp2006.mat")
...
for i = 1:length(Times)
    plot(Q_Qmax2(1:end-150,i),SqQmax2_2(1:end-150,i),strcat('-',markers{i}),'linewidth',1,'MarkerSize',8,'MarkerFaceColor',colors(i,:)) ; % 
    hold on
end

xxx = 0.8:1:30 ;
yyy = 5.0*10^4*xxx.^(-4.0) ;
...
text(4,500,'Slope $\sim -4.0$','fontsize',FS,'rotation',-53,'Interpreter','LaTex') ;
